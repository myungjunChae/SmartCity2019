...
let listener = (socket, device) => {
    ...
    if(device !== 'undefined' || device !== null){
        ...
        //Attach device Event Listener
        switch(device){
            case 'windows':
                windowsListener(socket, emitter);
                break;
            case 'android':
                androidListener(socket, emitter);
                break;
            default:
                break;
        }
...
let defaultListener = (socket, emitter) => {
    ...
    let injection = (ele, data, emitter) => {
        let payload = {device : device, broadcastType : ele.type};
       
        if(ele.event == 'EVENT_EXIT_ROOM'){
            payload['event']    = ele.event;
            payload['data']     = device;
        }else if( ele.event == 'EVENT_JOIN_ROOM'){
            socket.join('room1')

            payload['event']    = 'EVENT_JOIN_ROOM_HANDSHAKE';
            payload['data']     = device;
        }else if( ele.event == 'EVENT_JOIN_ROOM_HANDSHAKE'){
            payload['event']    = ele.event;
            payload['data']     = data;
        }
